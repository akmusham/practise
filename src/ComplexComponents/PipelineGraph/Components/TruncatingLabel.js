...
export class TruncatingLabel extends Component {
    ...
    renderState; // Internal rendering lifecycle state
    checkSizeRequest; // window.requestAnimationFrame handle

    //--------------------------------------
    //  Binary search state
    //--------------------------------------

    textCutoffLength; // Last count used to truncate completeText
    longestGood; // Length of the longest truncated text that fits
    shortestBad; // Length of the shortest truncated text that does not fit
    loopCount; // to avoid infinite iteration
    ...
    render() {
        ...
        return (
            <div style={mergedStyle} className={'TruncatingLabel ' + className}>{this.props.children}</div>
        );
    }
